...
<table id="sales" class="table table-striped table-hover table-bordered" style="width:100%">
    <thead>
    <tr>
        <th>#</th>
        <th>Tên</th>
    ...
    <c:forEach var="sale" items="${sales}">
        <tr>
            ...
            <td>
                <form action="${pageContext.request.contextPath}/dashboard/sales/${sale.id}" method="GET"
                      style="display:inline;">
                    <button type="submit" class="btn btn-info btn-sm">Xem</button>
                </form>
                <form action="${pageContext.request.contextPath}/dashboard/sales/${sale.id}/edit" method="GET"
                      style="display:inline;">
                    <button type="submit" class="btn btn-primary btn-sm">Sửa</button>
                </form>

                <form action="${pageContext.request.contextPath}/dashboard/sales/${sale.id}" method="POST" style="display:inline;"
        ...
        $('#sales').DataTable({
            ...
            layout: {
                topStart: {
                    buttons: [
                        {
                            text: '<i class="bi bi-person-add me-2"></i>Thêm chương trình giảm giá',
                            className: 'btn btn-secondary me-2',
                            action: function () {
                                window.location.href = '/dashboard/sales/create'; // Thay đổi URL đến trang tạo mới
                            }
                        },
                        {
                            extend: 'collection',
